...
import { listQuestions, getUserProfile, Question } from "@/src/lib/firestore";  
  ...
  async function loadQuestions() {  
    try {  
      ...
      const sorted = allQuestions.sort((a, b) => {  
        ...
        // Primero por estado de respuesta  
        if (aHasAnswer && !bHasAnswer) return -1;  
        if (!aHasAnswer && bHasAnswer) return 1;  
          
        // Luego por fecha de creación (más nuevas primero)  
      ...
      const uids = sorted.map(q => q.createdBy).filter(Boolean);  
      if (uids.length > 0) {  
        await loadUserNames(uids);  
      }  
    } catch (error) {  
                ...
                >  
                  <div style={{ marginBottom: '12px' }}>  
                    ...
                    <div style={{  
                      ...
                      Asked by {userNames[q.createdBy] || 'Anonymous'}  
                    </div>  
